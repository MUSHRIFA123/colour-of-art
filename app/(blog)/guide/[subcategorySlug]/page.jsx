...
const getPostsBySubcategory = async (categorySlug, subcategorySlug) => {
	const query = new URLSearchParams({
		category: categorySlug,
		subcategory: subcategorySlug,
		});
  const res = await fetch(`${process.env.API_URI}/api/posts-subcategory?${query.toString()}`);
...
export default async function BlogCategoryPage({ params }) {
	...
	const { posts } = await getPostsBySubcategory(categorySlug, subcategorySlug);

  return (
    <>
      <head>
        <title>{subcategorySlug}</title>
        <meta name="description" content={subcategorySlug} />
      </head>
      <body className={`${outfit.className} antialiased text-[rgba(0,0,0,0.95)]`}>
        <Navbar />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
          <h1 className="text-4xl font-bold text-center mb-10">{subcategorySlug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</h1>
